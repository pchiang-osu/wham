...
public func analyzeDetectionTimes(detectionTimes: [NSDate]) -> String {
    ...
    let averageBPM = heartbeatDifferences.reduce(0, combine: +) / Double(heartbeatDifferences.count)
    let averageVariability = variabilities.reduce(0, combine: +) / Double(variabilities.count)
    let beatsPerMinute = String(format:"%0.1f", 60 / averageBPM)
    
    var result = "Intervals: " + ", ".join(heartbeatDifferences.map({ String(format:"%0.2f", $0) }))
    result    += "\nVariabilities: "
    result    += ", ".join(variabilities.map({ String(format:"%0.2f", $0) }))
